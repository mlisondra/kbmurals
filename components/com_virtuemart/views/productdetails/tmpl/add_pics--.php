...
<span><strong><em>Upload files:</em></strong></span></br>  
...
<table width="100%" border="1" cellpadding="5" cellspacing="2">
  ...
  <tr>  
    <td colspan="3" height="35px" valign="bottom" align="left">
...
<label for="status"><?php echo VUOutput::_('YLANG_FORM_STATUS'); ?></label>
  ...
  <tr>
     <td colspan="3" >
         <input class="vu_status" size="30" type="text" id="status" name="status" readonly="readonly" value="<?php echo VUOutput::_('YLANG_STAT_WAITTUPL'); ?>" />
     </td>
 </tr>
<?php 
 if ($uploadinfo) {
...
//echo "SELECT * FROM  #__virtueuploads WHERE ((user_id <> 0 and user_id=".$userid.")  or session_id='".$_SESSION['sid_before_login']."' )	and session_id<>'finished' order by id desc";
   ...
   {
	  echo '<tr><td><input type=checkbox name=selectedfiles[] id=selectedfiles value='.$myrow_cur["id"].' checked></td>';
   }
   else
...
?>
<tr>
    <td height="50px" valign="top" align="right">
		<input id="delete_button" class="button" type="submit" value="<?php echo VUOutput::_('YLANG_DELETEFILE'); ?>" onclick="return( del_file( this.form ) );" />
	</td>
...
<input type="hidden" name="uri_url" value="<?php echo  VUOutput::_getUriString2(); ?>" />
