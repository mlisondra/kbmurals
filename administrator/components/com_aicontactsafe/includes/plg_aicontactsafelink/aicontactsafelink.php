			...
			for($i=1;$i<$no_links;$i++) {
				...
				if ( $this->checkProfile($parameters['pf']) ) {
					...
					if(!array_key_exists('action',$parameters)) {
						$parameters['action'] = 'index.php?option=com_aicontactsafe';
					...
					$plugin_link .= '<input type="hidden" name="lang" id="lang_'.$rnd_id.'" value="'.$lang.'" />';
					...
					if (!array_key_exists('return_to', $parameters)) {
						...
						$plugin_link .= '<input type="hidden" name="return_to" id="return_to_'.$rnd_id.'" value="'.$currentUrl.'" />';
					}
					// add all parameters to send them to the contact form ( except the text used as link )
					foreach($parameters as $key=>$value) {
						if ( $key != 'text' ) {
							$plugin_link .= '<input type="hidden" name="'.$key.'" id="'.$key.'_'.$rnd_id.'" value="'.$value.'" />';
						}
					}
					// add the token
